...
class RustAFKHourAdder:
    ...
    def __init__(self):
        ...
        self.settings = {
            ...
            "auto_download_missing_files": True,  # Auto-download missing data files from GitHub
        ...
        # Auto-download missing data files if enabled
        if self.settings.get("auto_download_missing_files", True):
            self.root.after(1000, self.check_and_download_missing_files)  # Check after 1 second
    ...
    def check_and_download_missing_files(self):
        """Check for missing data files and download them from GitHub if enabled"""
            ...
            try:
                ...
                for filename, url in required_files.items():
                    ...
                    if not os.path.exists(file_path):
                        missing_files.append((filename, url, file_path))
                
                if missing_files:
                    self.log_status(f"Auto-download: Found {len(missing_files)} missing data files")
                    
                    for filename, url, file_path in missing_files:
                        try:
                            self.log_status(f"Auto-download: Downloading {filename}...")
                            response = requests.get(url, timeout=10)
                            
                            if response.status_code == 200:
                                with open(file_path, 'w', encoding='utf-8') as f:
                                    f.write(response.text)
                                self.log_status(f"Auto-download: Successfully downloaded {filename}")
                            else:
                                self.log_status(f"Auto-download: Failed to download {filename}: HTTP {response.status_code}")
                                
                        except Exception as e:
                            self.log_status(f"Auto-download: Error downloading {filename}: {e}")
                    
                    # Reload servers if servers.json was downloaded
                    if any(filename == "servers.json" for filename, _, _ in missing_files):
                        self.servers = self.load_servers()
                        self.log_status("Auto-download: Reloaded server list")
                else:
                    self.log_status("Auto-download: All required data files are present")
                    
            except Exception as e:
                self.log_status(f"Auto-download check failed: {e}")
        
        # Run in background thread
        threading.Thread(target=download_missing_files, daemon=True).start()
...
Discord: https://discord.gg/a5T2xBhKgt"""
    ...
    def clear_validation_cache(self):
        """Manually clear all validation cache from servers"""
        cleared_count = 0
        for server in self.servers:
            keys_to_remove = ['_validation_method', '_battlemetrics_found', 'status', 'last_validated', 'validation_timestamp']
            for key in keys_to_remove:
                if key in server:
                    del server[key]
                    cleared_count += 1
        
        # Save the cleared servers
        self.save_servers()
        self.log_status(f"Manually cleared validation cache from {len(self.servers)} servers ({cleared_count} cached fields removed)")
    ...
    def validate_servers(self):
        ...
        if hasattr(self, '_validation_running') and self._validation_running:

            self.log_status("Validation already in progress - ignoring request")

            return

        

        # Always clear validation cache before starting new validation

        cleared_count = self.clear_validation_cache()

        self.log_status(f"Starting fresh validation - cleared {cleared_count} cached fields")
        ...
        battlemetrics_radio = tk.Radiobutton(method_frame, text="BattleMetrics API (Recommended - verifies actual server status)", 

                      variable=validation_method, value="battlemetrics")

        battlemetrics_radio.pack(anchor="w", pady=2)

        battlemetrics_radio.select()  # Explicitly select this option
        ...
        def start_validation():
            ...
            self.log_status(f"Validation state reset - starting fresh validation of {len(self.servers)} servers")
            ...
            for server in self.servers:
                ...
                # Remove all validation-related cached data
            ...
            # Force save the cleared servers to ensure cache is removed from file

            self.save_servers()

            self.log_status("Cleared all cached validation results - forcing fresh validation")

            

            # Debug: Count how many servers had cached data

            cached_servers = sum(1 for server in self.servers if any(key in server for key in ['_validation_method', '_battlemetrics_found', 'status']))

            if cached_servers > 0:

                self.log_status(f"WARNING: {cached_servers} servers still have cached validation data after clearing!")
            ...
            # Small delay to prevent rapid-fire validation
            ...
            self.log_status(f"Using validation method: {method}")
            ...
            def validate_thread():
                ...
                self.log_status(f"Validation thread started - method: {method}, servers: {len(self.servers)}")
                ...
                def validate_single_server(i, server):
                    ...
                    if method == "battlemetrics":
                        ...
                        if completed_servers < 5:  # Only log first 5 servers to avoid spam

                            self.log_status(f"Checking server {server_name} via BattleMetrics API...")
                        ...
                        if completed_servers < 5:  # Only log first 5 servers to avoid spam
                            ...
                            self.log_status(f"Pinging server {server_name} at {server_ip}...")

                        

                        # Add small delay to prevent ping validation from being too fast

                        import time

                        time.sleep(0.05)  # 50ms delay per server
    ...
    def create_basic_settings_tab(self):
        ...
        # Auto-download missing files settings
        self.auto_download_missing_files_var = tk.BooleanVar(value=self.settings.get("auto_download_missing_files", True))
        tk.Checkbutton(system_frame, text="Auto-download missing data files", 
                      variable=self.auto_download_missing_files_var, command=self.on_auto_download_missing_files_change).pack(anchor="w", pady=2)
        
        download_note = tk.Label(system_frame, text="Automatically download missing data files (servers.json, setup_windows_startup.bat) from GitHub", 
                                font=("Arial", 8), wraplength=350, fg="gray")
        download_note.pack(anchor="w", padx=20, pady=(0, 5))
    ...
    def initialize_gui_with_settings(self):
        ...
        try:
            ...
            self.auto_download_missing_files_var.set(self.settings.get("auto_download_missing_files", True))
    ...
    def on_auto_download_missing_files_change(self):
        """Handle auto-download missing files checkbox change"""
        self.settings["auto_download_missing_files"] = self.auto_download_missing_files_var.get()
        status = "ENABLED" if self.auto_download_missing_files_var.get() else "DISABLED"
        self.log_status(f"Auto-download missing data files: {status}")
        self.save_settings()
...
if errorlevel 1 (
    ...
    def reset_to_defaults(self):
        ...
        if messagebox.askyesno("Reset Settings", 
            ...
            self.auto_download_missing_files_var.set(True)
